...
**[Download the installer (v0.2.9, ~1.4 MB)](installer/Usage%20Widget%20for%20Claude_0.2.9_x64-setup.exe)** — Windows 10 / 11 x64, no admin rights needed. Requires the [WebView2 runtime](https://developer.microsoft.com/microsoft-edge/webview2/), which almost every Windows 10/11 machine already has (it ships with Windows 11 and with Edge on Windows 10) — if yours somehow doesn't, grab it from that link first.
