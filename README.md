...
**[Download the installer (v0.2.10, ~1.4 MB)](installer/Usage%20Widget%20for%20Claude_0.2.10_x64-setup.exe)** — Windows 10 / 11 x64, no admin rights needed. Requires the [WebView2 runtime](https://developer.microsoft.com/microsoft-edge/webview2/), which almost every Windows 10/11 machine already has (it ships with Windows 11 and with Edge on Windows 10) — if yours somehow doesn't, grab it from that link first.
...
Kani's mood tracks **whichever limit is currently highest** — session, weekly, or any per-model bucket, not just the weekly number — so if your session fills up first, Kani reacts to that before weekly usage even matters:

<table>
<tr>
<td align="center"><img src="assets/moods/happy.svg" width="150" alt="Happy Kani"><br><b>😊 Happy</b><br><sub>highest limit &lt; 50%</sub></td>
<td align="center"><img src="assets/moods/worried.svg" width="150" alt="Worried Kani"><br><b>😟 Worried</b><br><sub>50–80%</sub></td>
<td align="center"><img src="assets/moods/panic.svg" width="150" alt="Panic Kani"><br><b>😱 Panic</b><br><sub>over 80%</sub></td>
<td align="center"><img src="assets/moods/sleepy.svg" width="150" alt="Sleepy Kani"><br><b>😴 Sleepy</b><br><sub>error / expired token</sub></td>
</tr>
</table>
...
มาสคอตปูส้ม **Kani** จะเปลี่ยนอารมณ์ตาม limit ตัวที่สูงที่สุด ณ ขณะนั้น (session, weekly, หรือ per-model bucket ไหนก็ตามที่ใกล้เต็มที่สุด) **ไม่ใช่ดูแค่ weekly อย่างเดียว** — ดูหน้าตาแต่ละอารมณ์ได้ในตารางที่ [What it shows](#what-it-shows) ด้านบน
